...
//groudtruth_img_file如果不为NULL,则print准确率
DLL_API cv::Mat text_extract(Mat& img, char* classifier_dir, bool grey_result, char* groudtruth_img_file){
	Mat grey, lab_img, gradient_magnitude, segmentation, all_segmentations;

	vector<Region> regions;
	::MSER mser8(false, 25, 0.000008, 0.03, 1, 0.7);
	//classifier_dir = 'boost_train'
	string regionClassifierFile(classifier_dir);
	string groupClassifierFile = regionClassifierFile + "/trained_boost_groups.xml";
	regionClassifierFile += "/trained_boost_char.xml";

	RegionClassifier region_boost((char*)regionClassifierFile.c_str(), 0);
	GroupClassifier  group_boost((char*)groupClassifierFile.c_str(), &region_boost);
	...
	for (int step = 1; step<3; step++)
	{
		...
		if (step == 2)
		{
			...
			if (groudtruth_img_file != NULL)
			{
				cvtColor(segmentation, grey, CV_BGR2GRAY);
				threshold(grey, grey, 1, 255, CV_THRESH_BINARY);
				Mat gt;
				gt = imread(groudtruth_img_file);
				...
				cout << "Pixel level precision = " << (float)countNonZero(tmp_mask) / countNonZero(grey) << endl;
			}
			/*else
			{
			imshow("Original", img);
	...
	if (grey_result)
		return grey;
	else return segmentation;
}
DLL_API cv::Mat text_extract(char* img_file, char* classifier_dir, bool grey_result, char* groudtruth_img_file)
{
	cv::Mat img = imread(img_file);
	return text_extract(img, classifier_dir, grey_result, groudtruth_img_file);
}
int main(int argc, char** argv)
{
	if (argc < 2)
	{
		fprintf(stderr, "Please specify the image file! \n");
		exit(-1);
	}
	cv::Mat img = imread(argv[1]);
	text_extract(img, "boost_train", false, 0);
}
